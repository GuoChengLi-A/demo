...
class TreeBuilder(base.TreeBuilder):
    ...
    def insertCommentInitial(self, data, user=None):
        assert user is None or user is self.document
        assert self.document._elementTree is None
        self.initial_comments.append(data)

    def insertCommentMain(self, data, user=None):
        if (user == self.document and
                self.document._elementTree.getroot()[-1].tag == comment_type):
            warnings.warn("lxml cannot represent adjacent comments beyond the root elements", DataLossWarning)
        super(TreeBuilder, self).insertComment(data, user)
