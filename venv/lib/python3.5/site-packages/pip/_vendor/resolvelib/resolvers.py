...
RequirementInformation = collections.namedtuple(
    "RequirementInformation", ["requirement", "user"]
)
...
class Criterion(object):
    ...
    * `candidates` is a collection containing all possible candidates deducted
    ...
    def __repr__(self):
        requirements = ", ".join(
            "({!r}, via={!r})".format(req, user)
            for req, user in self.information
        )
        return "Criterion({})".format(requirements)

    @classmethod
    def from_requirement(cls, provider, requirement, user):
        ...
        criterion = cls(
            ...
            information=[RequirementInformation(requirement, user)],
            incompatibilities=[],
        )
        if not candidates:
    ...
    def iter_parent(self):
        return (i.user for i in self.information)

    def merged_with(self, provider, requirement, user):
        """Build a new instance from this and a new requirement.
        """
        infos = list(self.information)
        infos.append(RequirementInformation(requirement, user))
...
class Resolution(object):
    ...
    def _merge_into_criterion(self, requirement, user):
        self._r.adding_requirement(requirement, user)
        name = self._p.identify(requirement)
        try:
            crit = self.state.criteria[name]
        except KeyError:
            crit = Criterion.from_requirement(self._p, requirement, user)
        else:
            crit = crit.merged_with(self._p, requirement, user)
        return name, crit
    ...
    def _get_criteria_to_update(self, candidate):
        ...
        for r in self._p.get_dependencies(candidate):
            name, crit = self._merge_into_criterion(r, user=candidate)
            criteria[name] = crit
        return criteria
    ...
    def resolve(self, requirements, max_rounds):
        ...
        for r in requirements:
            try:
                name, crit = self._merge_into_criterion(r, user=None)
            except RequirementsConflicted as e:
...
def _build_result(state):
    ...
    graph = DirectedGraph()
    graph.add(None)  # Sentinel as root dependencies' user.
...
class Resolver(AbstractResolver):
    ...
    def resolve(self, requirements, max_rounds=100):
        ...
        * `ResolutionImpossible`: A resolution cannot be found for the given
            ...
            exception is a list of (requirement, user), giving the
