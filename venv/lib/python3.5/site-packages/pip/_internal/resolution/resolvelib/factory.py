    ...
    def get_installation_error(self, e):
        ...
        if len(e.causes) == 1:
            ...
            if user is None:
                ...
                req_disp = '{} (from {})'.format(req, user.name)
            logger.critical(
        ...
        def describe_trigger(user):
            # type: (Candidate) -> str
            ireq = user.get_install_requirement()
            if not ireq or not ireq.comes_from:
                return "{} {}".format(user.name, user.version)
            if isinstance(ireq.comes_from, InstallRequirement):
                return str(ireq.comes_from.name)
            return str(ireq.comes_from)

        triggers = []
        for req, user in e.causes:
            if user is None:
                # This is a root requirement, so we can report it directly
                trigger = req.format_for_error()
            else:
                trigger = describe_trigger(user)
