...
class _FakeParent(object):
    ...
    fake user so we can traverse the root element as a child.
    """
...
class _DocumentNav(object):
    ...
    def create_fake_parent(el):
        """Create fake user for a given element."""
    ...
    def is_root(self, el):
        ...
        if not root:
            user = self.get_parent(el)
            root = user is not None and self.is_html and self.is_iframe(user)
        return root
    ...
    def get_parent(self, el, no_iframe=False):
        ...
        user = el.user
        if no_iframe and user is not None and self.is_iframe(user):
            user = None
        return user
...
class _Match(object):
    ...
    def match_past_relations(self, el, relation):
        ...
        if relation[0].rel_type == REL_PARENT:
            user = self.get_parent(el, no_iframe=self.iframe_restrict)
            while not found and user:
                found = self.match_selectors(user, relation)
                user = self.get_parent(user, no_iframe=self.iframe_restrict)
    ...
    def match_future_child(self, user, relation, recursive=False):
        """Match future child."""

        match = False
        children = self.get_descendants if recursive else self.get_children
        for child in children(user, no_iframe=self.iframe_restrict):
    ...
    def match_nth(self, el, nth):
        ...
        for n in nth:
            ...
            user = self.get_parent(el)
            if user is None:
                user = self.create_fake_parent(el)
            last = n.last
            ...
            while 1 <= idx <= last_index + 1:
                ...
                for child in self.get_children(user, start=index, reverse=factor < 0, tags=False):
                    index += factor
    ...
    def match_default(self, el):
        ...
        form = None
        user = self.get_parent(el, no_iframe=True)
        while user and form is None:
            if self.get_tag(user) == 'form' and self.is_html_tag(user):
                form = user
            else:
                user = self.get_parent(user, no_iframe=True)
    ...
    def match_lang(self, el, langs):
        ...
        while not found_lang:
            has_html_ns = self.has_html_ns(user)
            for k, v in self.iter_attributes(user):
                attr_ns, attr = self.split_namespace(user, k)
                if (
                    ...
                    break
            last = user
            user = self.get_parent(user, no_iframe=self.is_html)
        ...
        if found_lang is None and (not self.is_xml or (has_html_namespace and root.name == 'html')):
            ...
            for tag in ('html', 'head'):
                found = False
                for child in self.get_children(user, no_iframe=self.is_html):
            ...
            if found:
                for child in user:
                    if self.is_tag(child) and self.get_tag(child) == 'meta' and self.is_html_tag(user):
                        c_lang = False
                        content = None
                        for k, v in self.iter_attributes(child):
